...
def mail_code(request):
    list = clients.objects.filter(address=request.POST.get('address'))
    if len(list):
    ...
    message['From'] = Header("欢迎注册网吧管理系统", 'utf-8')
    ...
    try:
        ...
        sql = vertify(address=receiver, code=code, time=str(int(time.time())))
        sql.save()
